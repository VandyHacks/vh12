    ...
    return (
        <div className="flex flex-col items-center text-center text-sm md:text-lg">
            <p className={textWrap ? "" : "text-nowrap"}>
                {label}{required && <span className="text-red-500 relative -translate-y-2 inline-block flex-inline">{"*"}</span>}
            </p>
            <Controller name={name} control={control} rules={rules} render={({ field }) => {
                ...
                return <div className="flex flex-row gap-3 flex-wrap justify-center items-center mt-3">
                    {
                        options.map((item) => {
                            return (
                                <div key={item} onClick={() => handleClick(item)} className={cn("px-4 active:translate-y-0.5 cursor-pointer text-[10px] md:text-[15px] py-3 text-black cursor-pointer shadow-[6px_6px_0px_0px_#000]", isSelected(item) ? "bg-blue-300" : " bg-stone-200")}>
                                    {item}
                                </div>
                            );
                        })
            ...
            {
                error && <p className="text-[8px] md:text-sm text-red-500 mt-2.5 ml-1">{error.message}</p>
            }
        </div>
    );
}