...
export default function Selector({ name, label, options, control, error, required = true, fieldRef, multiple = false, textWrap = false, _rules }: { name: string, label: string | ReactNode, options: string[], control: any, fieldRef: any, error?: any, multiple?: boolean, required?: boolean, textWrap?: boolean, _rules?: any }) {
    ...
    return (
        <div ref={fieldRef} className="flex flex-col items-center text-center text-sm md:text-lg py-2">
                        ...
                        options.map((item) => {
                            return (
                                <motion.div 
                                    ...
                                    className="px-4 active:translate-y-0.5 cursor-pointer text-[10px] md:text-[15px] py-3 text-black cursor-pointer shadow-[6px_6px_0px_0px_#000] hover:brightness-95 transition-[filter] duration-150 select-none"
                                    animate={{
